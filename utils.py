...
def grad_cam_func(model, misclassified_images):
    ...
    for img, pred, target in misclassified_images:
        ...
        input_tensor = preprocessed_input.permute(2, 0, 1).unsqueeze(0).to(device)
        
        # Define the target category for this image
        target_category = target.item()  # Use the actual target label
        
        # Define the target for GradCAM
        targets = [target_category]

        # Assuming 'input_tensor' is the tensor you are passing to the model
        print("Shape of input tensor:", input_tensor.shape)
        
        # Assuming 'model' is your neural network model
        # You can access the expected input shape from the first convolutional layer
        # For example, if your model is a CNN and the first layer is named 'conv1'
        expected_input_shape = model.conv1.in_channels  # Get the number of input channels expected by the first layer
        print("Expected input shape by the model:", expected_input_shape)
        ...
        visualization = visualize_cam(mask=grayscale_cam[0], img=torch.tensor(img_np).permute(2, 0, 1))
